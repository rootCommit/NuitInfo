...
$cookieName="cookIdent";
    ...
    if (!empty($_POST)){ //On verifie si le formulaire a bien ete soumis 
        $query = "SELECT * FROM identifiants where identifiant = :identifiant AND motdepasse = :password ";
        $req = $conn->prepare($query);
        $identifiant = htmlspecialchars($_POST['identifiant']);
        $password = htmlspecialchars($_POST['password']);
        $req->execute(array('identifiant' => $identifiant, 'password' =>  $password));
        $resultat = $req->fetch(); 
		if(empty($_POST['identifiant']) || empty($_POST['password']) || !$resultat){ 
		//si il y a une anomalie dans l'entrée de formulaire
			while($resultat){
				echo $resultat['identifiant'];
			}
			header('Location: ../formulaire.php?error='.$error.''); // on renvoie notre erreur au formulaire via la methode GET
		}
		else{ // cas où l’identification est correcte
			session_start();	//Creation de la session
			$_SESSION['identified'] = "OK";
			$_SESSION['login'] = $identifiant;
			$_SESSION['id'] = $resultat['idIdentifiants'];
			echo "ca marche ";
		...
		if(isset($_COOOKIE[$cookieName])){
			...
			header('Location: ../formulaire.php?error='.$error.'');
		}
	}
